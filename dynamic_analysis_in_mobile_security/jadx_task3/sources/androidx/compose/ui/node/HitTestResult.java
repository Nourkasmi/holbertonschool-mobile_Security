...
public final class HitTestResult implements List<Modifier.Node>, KMappedMarker {
    ...
    public final boolean hasHit() {
        long distance = m4316findBestHitDistanceptXAw2c();
        return DistanceAndInLayer.m4311getDistanceimpl(distance) < 0.0f && DistanceAndInLayer.m4313isInLayerimpl(distance);
    }
    ...
    public final boolean isHitInMinimumTouchTargetBetter(float distanceFromEdge, boolean isInLayer) {
        ...
        long distanceAndInLayer = HitTestResultKt.DistanceAndInLayer(distanceFromEdge, isInLayer);
        long bestDistance = m4316findBestHitDistanceptXAw2c();
        return DistanceAndInLayer.m4307compareToS_HNhKs(bestDistance, distanceAndInLayer) > 0;
    }

    /* JADX INFO: renamed from: findBestHitDistance-ptXAw2c, reason: not valid java name */
    private final long m4316findBestHitDistanceptXAw2c() {
        long bestDistance = HitTestResultKt.DistanceAndInLayer(Float.POSITIVE_INFINITY, false);
        int i = this.hitDepth + 1;
        int lastIndex = CollectionsKt.getLastIndex(this);
        if (i <= lastIndex) {
            while (true) {
                long distance = DistanceAndInLayer.m4308constructorimpl(this.distanceFromEdgeAndInLayer[i]);
                bestDistance = DistanceAndInLayer.m4307compareToS_HNhKs(distance, bestDistance) < 0 ? distance : bestDistance;
                if (DistanceAndInLayer.m4311getDistanceimpl(bestDistance) >= 0.0f || !DistanceAndInLayer.m4313isInLayerimpl(bestDistance)) {
                    if (i == lastIndex) {
                        break;
                    }
                    i++;
    ...
    public final void speculativeHit(Modifier.Node node, float distanceFromEdge, boolean isInLayer, Function0<Unit> childHitTest) {
        ...
        long previousDistance = m4316findBestHitDistanceptXAw2c();
        int previousHitDepth = this.hitDepth;
        this.hitDepth = CollectionsKt.getLastIndex(this);
        hitInMinimumTouchTarget(node, distanceFromEdge, isInLayer, childHitTest);
        if (this.hitDepth + 1 < CollectionsKt.getLastIndex(this) && DistanceAndInLayer.m4307compareToS_HNhKs(previousDistance, m4316findBestHitDistanceptXAw2c()) > 0) {
    ...
    private final void ensureContainerSize() {
        ...
        if (i >= objArr.length) {
            ...
            Object[] objArrCopyOf = Arrays.copyOf(objArr, newSize);
            Intrinsics.checkNotNullExpressionValue(objArrCopyOf, "copyOf(this, newSize)");
            this.values = objArrCopyOf;
            long[] jArrCopyOf = Arrays.copyOf(this.distanceFromEdgeAndInLayer, newSize);
