...
public abstract class AlignmentLines {
    ...
    protected abstract long mo4280calculatePositionInParentR5De75A(NodeCoordinator nodeCoordinator, long j);
    ...
    public final void addAlignmentLine(AlignmentLine alignmentLine, int initialPosition, NodeCoordinator initialCoordinator) {
        ...
        while (true) {
            position = mo4280calculatePositionInParentR5De75A(coordinator, position);
        ...
        if (alignmentLine instanceof HorizontalAlignmentLine) {
            positionInContainer = MathKt.roundToInt(Offset.m2709getYimpl(position));
        } else {
            positionInContainer = MathKt.roundToInt(Offset.m2708getXimpl(position));
        }
        Map<AlignmentLine, Integer> map = this.alignmentLineMap;
        if (map.containsKey(alignmentLine)) {
            iMerge = AlignmentLineKt.merge(alignmentLine, ((Number) MapsKt.getValue(this.alignmentLineMap, alignmentLine)).intValue(), positionInContainer);
        } else {
            iMerge = positionInContainer;
        }
