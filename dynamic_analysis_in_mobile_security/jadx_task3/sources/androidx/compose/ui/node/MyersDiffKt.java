...
public final class MyersDiffKt {
    private static final IntStack calculateDiff(int oldSize, int newSize, DiffCallback cb) {
        ...
        int[] forward = CenteredArray.m4283constructorimpl(new int[(oldEnd * 2) + 1]);
        int[] backward = CenteredArray.m4283constructorimpl(new int[(oldEnd * 2) + 1]);
        int[] snake2 = Snake.m4457constructorimpl(new int[5]);
        while (stack.isNotEmpty()) {
            ...
            int[] snake3 = snake2;
            boolean found = m4359midPointq5eDKzI(oldStart, oldEnd2, newStart, newEnd, cb, forward, backward, snake2);
            if (!found) {
                snake2 = snake3;
                oldEnd = max;
                forward = forward2;
            } else {
                if (Snake.m4460getDiagonalSizeimpl(snake3) <= 0) {
                    snake = snake3;
                } else {
                    snake = snake3;
                    Snake.m4455addDiagonalToStackimpl(snake, diagonals);
                }
                stack.pushRange(oldStart, Snake.m4465getStartXimpl(snake), newStart, Snake.m4466getStartYimpl(snake));
                stack.pushRange(Snake.m4461getEndXimpl(snake), oldEnd2, Snake.m4462getEndYimpl(snake), newEnd);
                snake2 = snake;
                oldEnd = max;
                forward = forward2;
            }
    ...
    private static final boolean m4359midPointq5eDKzI(int oldStart, int oldEnd, int newStart, int newEnd, DiffCallback cb, int[] forward, int[] backward, int[] snake) {
        ...
        CenteredArray.m4289setimpl(forward, 1, oldStart);
        CenteredArray.m4289setimpl(backward, 1, oldEnd);
        int d = 0;
        while (d < max) {
            int d2 = d;
            boolean found = m4358forward4l5_RBY(oldStart, oldEnd, newStart, newEnd, cb, forward, backward, d2, snake);
            if (found) {
                return true;
            }
            boolean found2 = m4357backward4l5_RBY(oldStart, oldEnd, newStart, newEnd, cb, forward, backward, d2, snake);
            if (found2) {
                return true;
            }
            d = d2 + 1;
    ...
    private static final boolean m4358forward4l5_RBY(int oldStart, int oldEnd, int newStart, int newEnd, DiffCallback cb, int[] forward, int[] backward, int d, int[] snake) {
        int startX;
        int x;
        int i = oldEnd;
        ...
        int k = -d;
        while (k <= d) {
            if (k == (-d) || (k != d && CenteredArray.m4286getimpl(forward, k + 1) > CenteredArray.m4286getimpl(forward, k - 1))) {
                startX = CenteredArray.m4286getimpl(forward, k + 1);
                x = startX;
            } else {
                startX = CenteredArray.m4286getimpl(forward, k - 1);
                x = startX + 1;
            ...
            CenteredArray.m4289setimpl(forward, k, x);
            if (checkForSnake) {
                int backwardsK = delta - k;
                if (backwardsK >= (-d) + i2 && backwardsK <= d - 1) {
                    if (CenteredArray.m4286getimpl(backward, backwardsK) > x) {
                        i2 = 1;
                    } else {
    ...
    private static final boolean m4357backward4l5_RBY(int oldStart, int oldEnd, int newStart, int newEnd, DiffCallback cb, int[] forward, int[] backward, int d, int[] snake) {
        int startX;
        int x;
        int oldSize = oldEnd - oldStart;
        int newSize = newEnd - newStart;
        ...
        int delta = oldSize - newSize;
        for (int k = -d; k <= d; k += 2) {
            if (k == (-d) || (k != d && CenteredArray.m4286getimpl(backward, k + 1) < CenteredArray.m4286getimpl(backward, k - 1))) {
                startX = CenteredArray.m4286getimpl(backward, k + 1);
                x = startX;
            } else {
                startX = CenteredArray.m4286getimpl(backward, k - 1);
                x = startX - 1;
            ...
            CenteredArray.m4289setimpl(backward, k, x);
            if (checkForSnake) {
                int forwardsK = delta - k;
                if (forwardsK >= (-d) && forwardsK <= d && CenteredArray.m4286getimpl(forward, forwardsK) >= x) {
