...
public final class OutlineResolver {
    ...
    public OutlineResolver(Density density) {
        ...
        this.size = Size.INSTANCE.m2786getZeroNHjbRc();
        this.shape = RectangleShapeKt.getRectangleShape();
        this.rectTopLeft = Offset.INSTANCE.m2724getZeroF1C5BW0();
        this.rectSize = Size.INSTANCE.m2786getZeroNHjbRc();
    ...
    public final boolean m4537isInOutlinek4lQ0M(long position) {
        androidx.compose.ui.graphics.Outline outline;
        if (this.outlineNeeded && (outline = this.calculatedOutline) != null) {
            return ShapeContainingUtilKt.isInOutline(outline, Offset.m2708getXimpl(position), Offset.m2709getYimpl(position), this.tmpTouchPointPath, this.tmpOpPath);
        }
        return true;
    }
    ...
    public final void clipToOutline(Canvas canvas) {
        ...
        if (targetPath != null) {
            Canvas.m2918clipPathmtrdDE$default(canvas, targetPath, 0, 2, null);
            return;
        }
        ...
        if (f > 0.0f) {
            ...
            if (roundRectClipPath == null || !m4536isSameBounds4L21HEs(roundRect, this.rectTopLeft, this.rectSize, f)) {
                RoundRect roundRect2 = RoundRectKt.m2762RoundRectgG7oq9Y(Offset.m2708getXimpl(this.rectTopLeft), Offset.m2709getYimpl(this.rectTopLeft), Offset.m2708getXimpl(this.rectTopLeft) + Size.m2777getWidthimpl(this.rectSize), Offset.m2709getYimpl(this.rectTopLeft) + Size.m2774getHeightimpl(this.rectSize), CornerRadiusKt.CornerRadius$default(this.roundedCornerRadius, 0.0f, 2, null));
            ...
            Canvas.m2918clipPathmtrdDE$default(canvas, roundRectClipPath, 0, 2, null);
            return;
        }
        Canvas.m2919clipRectN_I0leg$default(canvas, Offset.m2708getXimpl(this.rectTopLeft), Offset.m2709getYimpl(this.rectTopLeft), Offset.m2708getXimpl(this.rectTopLeft) + Size.m2777getWidthimpl(this.rectSize), Offset.m2709getYimpl(this.rectTopLeft) + Size.m2774getHeightimpl(this.rectSize), 0, 16, null);
    }

    /* JADX INFO: renamed from: update-uvyYCjk, reason: not valid java name */
    public final void m4538updateuvyYCjk(long size) {
        if (!Size.m2773equalsimpl0(this.size, size)) {
            this.size = size;
            this.cacheIsDirty = true;
        }
    }
    ...
    private final void updateCache() {
        if (this.cacheIsDirty) {
            ...
            if (this.outlineNeeded && Size.m2777getWidthimpl(j) > 0.0f && Size.m2774getHeightimpl(this.size) > 0.0f) {
                this.isSupportedOutline = true;
    ...
    private final void updateCacheWithRoundRect(RoundRect roundRect) {
        float radius = CornerRadius.m2683getXimpl(roundRect.m2758getTopLeftCornerRadiuskKHJgLs());
    ...
    private final boolean m4536isSameBounds4L21HEs(RoundRect $this$isSameBounds_u2d4L21HEs, long offset, long size, float radius) {
        if ($this$isSameBounds_u2d4L21HEs == null || !RoundRectKt.isSimple($this$isSameBounds_u2d4L21HEs)) {
            return false;
        }
        if ($this$isSameBounds_u2d4L21HEs.getLeft() == Offset.m2708getXimpl(offset)) {
            if ($this$isSameBounds_u2d4L21HEs.getTop() == Offset.m2709getYimpl(offset)) {
                if ($this$isSameBounds_u2d4L21HEs.getRight() == Offset.m2708getXimpl(offset) + Size.m2777getWidthimpl(size)) {
                    if ($this$isSameBounds_u2d4L21HEs.getBottom() == Offset.m2709getYimpl(offset) + Size.m2774getHeightimpl(size)) {
                        return (CornerRadius.m2683getXimpl($this$isSameBounds_u2d4L21HEs.m2758getTopLeftCornerRadiuskKHJgLs()) > radius ? 1 : (CornerRadius.m2683getXimpl($this$isSameBounds_u2d4L21HEs.m2758getTopLeftCornerRadiuskKHJgLs()) == radius ? 0 : -1)) == 0;
                    }
                    return false;
                }
                return false;
