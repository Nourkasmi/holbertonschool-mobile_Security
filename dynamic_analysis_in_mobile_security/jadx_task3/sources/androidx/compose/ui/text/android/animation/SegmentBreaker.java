...
public final class SegmentBreaker {
    ...
    private final List<Integer> breakInWords(LayoutHelper layoutHelper) {
        ...
        for (int index$iv = 0; index$iv < size; index$iv++) {
            Object item$iv = listBreakWithBreakIterator.get(index$iv);
        ...
        switch (WhenMappings.$EnumSwitchMapping$0[segmentType.ordinal()]) {
            ...
            case 2:
                ...
                for (int i = 0; i < paragraphCount; i++) {
                    listMutableListOf.add(Integer.valueOf(layoutHelper.getParagraphEnd(i)));
                }
                return listMutableListOf;
            case 3:
    ...
    private final List<Segment> breakSegmentWithWord(LayoutHelper layoutHelper, boolean dropSpaces) {
        ...
        List<Integer> listBreakOffsets = breakOffsets(layoutHelper2, SegmentType.Word);
        int $i$f$fastZipWithNext = 0;
        if (listBreakOffsets.size() == 0 || listBreakOffsets.size() == 1) {
            return CollectionsKt.emptyList();
        }
        List result$iv = new ArrayList();
        boolean z = false;
        Object current$iv = listBreakOffsets.get(0);
        int i$iv = 0;
        int lastIndex = CollectionsKt.getLastIndex(listBreakOffsets);
        while (i$iv < lastIndex) {
            Object next$iv = listBreakOffsets.get(i$iv + 1);
            ...
            List<Integer> list = listBreakOffsets;
            ...
            result$iv2.add(new Segment(start, end, left, layout.getLineTop(lineNo), right, layout.getLineBottom(lineNo)));
            current$iv = next$iv;
            i$iv++;
            layoutHelper2 = layoutHelper;
            result$iv = result$iv2;
            $i$f$fastZipWithNext = $i$f$fastZipWithNext2;
            listBreakOffsets = list;
            z = false;
        }
        return result$iv;
    }
