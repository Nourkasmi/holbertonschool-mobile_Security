...
public final class Vertices {
    ...
    private Vertices(int vertexMode, final List<Offset> positions, List<Offset> textureCoordinates, List<Color> colors, List<Integer> indices) {
        ...
        while (true) {
            if (index$iv$iv < size) {
                if (function1.invoke(indices.get(index$iv$iv)).booleanValue()) {
                    z = true;
                    break;
                }
                index$iv$iv++;
    ...
    private final float[] encodePointList(List<Offset> points) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (i % 2 == 0) {
                fM2709getYimpl = Offset.m2708getXimpl(point);
            } else {
                fM2709getYimpl = Offset.m2709getYimpl(point);
            }
            fArr[i] = fM2709getYimpl;
        }
        return fArr;
    }
}
