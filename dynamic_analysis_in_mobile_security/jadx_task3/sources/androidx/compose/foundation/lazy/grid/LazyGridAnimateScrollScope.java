...
public final class LazyGridAnimateScrollScope implements LazyAnimateScrollScope {
    ...
    public Integer getTargetItemOffset(int index) {
        ...
        if (item != null) {
            return Integer.valueOf(this.state.getIsVertical() ? IntOffset.m5331getYimpl(item.getOffset()) : IntOffset.m5330getXimpl(item.getOffset()));
        }
        return null;
    }
        ...
        while (lineStartIndex < visibleItemsInfo.size()) {
            ...
            if (currentLine == -1) {
                ...
                while (lineEndIndex < visibleItemsInfo.size() && function1.invoke(Integer.valueOf(lineEndIndex)).intValue() == currentLine) {
                    if (isVertical) {
                        iM5372getWidthimpl = IntSize.m5371getHeightimpl(visibleItemsInfo.get(lineEndIndex).getSize());
                    } else {
                        iM5372getWidthimpl = IntSize.m5372getWidthimpl(visibleItemsInfo.get(lineEndIndex).getSize());
                    }
                    lineMainAxisSize = Math.max(lineMainAxisSize, iM5372getWidthimpl);
                    lineEndIndex++;
                }
