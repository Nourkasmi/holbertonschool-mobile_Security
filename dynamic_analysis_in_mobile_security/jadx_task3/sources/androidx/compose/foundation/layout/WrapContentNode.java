...
final class WrapContentNode extends Modifier.Node implements LayoutModifierNode {
    ...
    public MeasureResult mo238measure3p2s80s(final MeasureScope measure, Measurable measurable, long constraints) {
        ...
        int iM5170getMinWidthimpl = this.direction != Direction.Vertical ? 0 : Constraints.m5170getMinWidthimpl(constraints);
        int iM5169getMinHeightimpl = this.direction == Direction.Horizontal ? Constraints.m5169getMinHeightimpl(constraints) : 0;
        if (this.direction != Direction.Vertical && this.unbounded) {
            iM5168getMaxWidthimpl = Integer.MAX_VALUE;
        } else {
            iM5168getMaxWidthimpl = Constraints.m5168getMaxWidthimpl(constraints);
        }
        long wrappedConstraints = ConstraintsKt.Constraints(iM5170getMinWidthimpl, iM5168getMaxWidthimpl, iM5169getMinHeightimpl, (this.direction == Direction.Horizontal || !this.unbounded) ? Constraints.m5167getMaxHeightimpl(constraints) : Integer.MAX_VALUE);
        final Placeable placeable = measurable.mo4183measureBRTryo0(wrappedConstraints);
        final int wrapperWidth = RangesKt.coerceIn(placeable.getWidth(), Constraints.m5170getMinWidthimpl(constraints), Constraints.m5168getMaxWidthimpl(constraints));
        final int wrapperHeight = RangesKt.coerceIn(placeable.getHeight(), Constraints.m5169getMinHeightimpl(constraints), Constraints.m5167getMaxHeightimpl(constraints));
        return MeasureScope.layout$default(measure, wrapperWidth, wrapperHeight, null, new Function1<Placeable.PlacementScope, Unit>() { // from class: androidx.compose.foundation.layout.WrapContentNode$measure$1
            ...
            public final void invoke2(Placeable.PlacementScope layout) {
                ...
                long position = this.this$0.getAlignmentCallback().invoke(IntSize.m5364boximpl(IntSizeKt.IntSize(wrapperWidth - placeable.getWidth(), wrapperHeight - placeable.getHeight())), measure.getLayoutDirection()).getPackedValue();
