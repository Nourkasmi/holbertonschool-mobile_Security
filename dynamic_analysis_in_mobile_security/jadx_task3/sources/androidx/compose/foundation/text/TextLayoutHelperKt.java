...
public final class TextLayoutHelperKt {
    ...
    public static final boolean m832canReuse7_7YC6M(TextLayoutResult canReuse, AnnotatedString text, TextStyle style, List<AnnotatedString.Range<Placeholder>> placeholders, int maxLines, boolean softWrap, int overflow, Density density, LayoutDirection layoutDirection, FontFamily.Resolver fontFamilyResolver, long constraints) {
        ...
        if (canReuse.getMultiParagraph().getIntrinsics().getHasStaleResolvedFonts() || !Intrinsics.areEqual(layoutInput.getText(), text) || !layoutInput.getStyle().hasSameLayoutAffectingAttributes(style) || !Intrinsics.areEqual(layoutInput.getPlaceholders(), placeholders) || layoutInput.getMaxLines() != maxLines || layoutInput.getSoftWrap() != softWrap || !TextOverflow.m5124equalsimpl0(layoutInput.getOverflow(), overflow) || !Intrinsics.areEqual(layoutInput.getDensity(), density) || layoutInput.getLayoutDirection() != layoutDirection || !Intrinsics.areEqual(layoutInput.getFontFamilyResolver(), fontFamilyResolver) || Constraints.m5170getMinWidthimpl(constraints) != Constraints.m5170getMinWidthimpl(layoutInput.getConstraints())) {
            return false;
        }
        if (softWrap || TextOverflow.m5124equalsimpl0(overflow, TextOverflow.INSTANCE.m5132getEllipsisgIe3tQ8())) {
            return Constraints.m5168getMaxWidthimpl(constraints) == Constraints.m5168getMaxWidthimpl(layoutInput.getConstraints()) && Constraints.m5167getMaxHeightimpl(constraints) == Constraints.m5167getMaxHeightimpl(layoutInput.getConstraints());
        }
        return true;
    }
}
