...
public final class TextDelegate {
    ...
    public /* synthetic */ TextDelegate(AnnotatedString annotatedString, TextStyle textStyle, int i, int i2, boolean z, int i3, Density density, FontFamily.Resolver resolver, List list, int i4, DefaultConstructorMarker defaultConstructorMarker) {
        this(annotatedString, textStyle, (i4 & 4) != 0 ? Integer.MAX_VALUE : i, (i4 & 8) != 0 ? 1 : i2, (i4 & 16) != 0 ? true : z, (i4 & 32) != 0 ? TextOverflow.INSTANCE.m5131getClipgIe3tQ8() : i3, density, resolver, (i4 & 256) != 0 ? CollectionsKt.emptyList() : list, null);
    }
    ...
    private final MultiParagraph m797layoutTextK40F9xA(long constraints, LayoutDirection layoutDirection) {
        int maxWidth;
        int width;
        layoutIntrinsics(layoutDirection);
        int minWidth = Constraints.m5170getMinWidthimpl(constraints);
        boolean overwriteMaxLines = false;
        boolean widthMatters = this.softWrap || TextOverflow.m5124equalsimpl0(this.overflow, TextOverflow.INSTANCE.m5132getEllipsisgIe3tQ8());
        if (widthMatters && Constraints.m5164getHasBoundedWidthimpl(constraints)) {
            maxWidth = Constraints.m5168getMaxWidthimpl(constraints);
        } else {
            maxWidth = Integer.MAX_VALUE;
        }
        if (!this.softWrap && TextOverflow.m5124equalsimpl0(this.overflow, TextOverflow.INSTANCE.m5132getEllipsisgIe3tQ8())) {
        ...
        return new MultiParagraph(getNonNullIntrinsics(), ConstraintsKt.Constraints$default(0, width, 0, Constraints.m5167getMaxHeightimpl(constraints), 5, null), finalMaxLines, TextOverflow.m5124equalsimpl0(this.overflow, TextOverflow.INSTANCE.m5132getEllipsisgIe3tQ8()), null);
    }

    /* JADX INFO: renamed from: layout-NN6Ew-U$default, reason: not valid java name */
    public static /* synthetic */ TextLayoutResult m796layoutNN6EwU$default(TextDelegate textDelegate, long j, LayoutDirection layoutDirection, TextLayoutResult textLayoutResult, int i, Object obj) {
        if ((i & 4) != 0) {
            textLayoutResult = null;
        }
        return textDelegate.m799layoutNN6EwU(j, layoutDirection, textLayoutResult);
    }

    /* JADX INFO: renamed from: layout-NN6Ew-U, reason: not valid java name */
    public final TextLayoutResult m799layoutNN6EwU(long constraints, LayoutDirection layoutDirection, TextLayoutResult prevResult) {
        Intrinsics.checkNotNullParameter(layoutDirection, "layoutDirection");
        if (prevResult != null && TextLayoutHelperKt.m832canReuse7_7YC6M(prevResult, this.text, this.style, this.placeholders, this.maxLines, this.softWrap, this.overflow, this.density, layoutDirection, this.fontFamilyResolver, constraints)) {
            return prevResult.m4692copyO0kMr_c(new TextLayoutInput(prevResult.getLayoutInput().getText(), this.style, prevResult.getLayoutInput().getPlaceholders(), prevResult.getLayoutInput().getMaxLines(), prevResult.getLayoutInput().getSoftWrap(), prevResult.getLayoutInput().getOverflow(), prevResult.getLayoutInput().getDensity(), prevResult.getLayoutInput().getLayoutDirection(), prevResult.getLayoutInput().getFontFamilyResolver(), constraints, (DefaultConstructorMarker) null), ConstraintsKt.m5179constrain4WqzIAM(constraints, IntSizeKt.IntSize(TextDelegateKt.ceilToIntPx(prevResult.getMultiParagraph().getWidth()), TextDelegateKt.ceilToIntPx(prevResult.getMultiParagraph().getHeight()))));
        }
        MultiParagraph multiParagraph = m797layoutTextK40F9xA(constraints, layoutDirection);
        long size = ConstraintsKt.m5179constrain4WqzIAM(constraints, IntSizeKt.IntSize(TextDelegateKt.ceilToIntPx(multiParagraph.getWidth()), TextDelegateKt.ceilToIntPx(multiParagraph.getHeight())));
