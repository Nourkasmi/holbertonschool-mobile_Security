...
public interface SelectionAdjustment {
    ...
    long mo886adjustZXO7KMw(TextLayoutResult textLayoutResult, long newRawSelectionRange, int previousHandleOffset, boolean isStartHandle, TextRange previousSelectionRange);
    ...
    public static final class Companion {
        ...
        private static final SelectionAdjustment None = new SelectionAdjustment() { // from class: androidx.compose.foundation.text.selection.SelectionAdjustment$Companion$None$1
            ...
            public long mo886adjustZXO7KMw(TextLayoutResult textLayoutResult, long newRawSelectionRange, int previousHandleOffset, boolean isStartHandle, TextRange previousSelectionRange) {
        ...
        private static final SelectionAdjustment Character = new SelectionAdjustment() { // from class: androidx.compose.foundation.text.selection.SelectionAdjustment$Companion$Character$1
            ...
            public long mo886adjustZXO7KMw(TextLayoutResult textLayoutResult, long newRawSelectionRange, int previousHandleOffset, boolean isStartHandle, TextRange previousSelectionRange) {
                Intrinsics.checkNotNullParameter(textLayoutResult, "textLayoutResult");
                if (TextRange.m4715getCollapsedimpl(newRawSelectionRange)) {
                    boolean previousHandlesCrossed = previousSelectionRange != null ? TextRange.m4720getReversedimpl(previousSelectionRange.getPackedValue()) : false;
                    return SelectionAdjustmentKt.ensureAtLeastOneChar(textLayoutResult.getLayoutInput().getText().getText(), TextRange.m4721getStartimpl(newRawSelectionRange), StringsKt.getLastIndex(textLayoutResult.getLayoutInput().getText()), isStartHandle, previousHandlesCrossed);
                }
                return newRawSelectionRange;
            }
        };
        private static final SelectionAdjustment Word = new SelectionAdjustment() { // from class: androidx.compose.foundation.text.selection.SelectionAdjustment$Companion$Word$1
            ...
            public long mo886adjustZXO7KMw(TextLayoutResult textLayoutResult, long newRawSelectionRange, int previousHandleOffset, boolean isStartHandle, TextRange previousSelectionRange) {
                Intrinsics.checkNotNullParameter(textLayoutResult, "textLayoutResult");
                return SelectionAdjustment.Companion.$$INSTANCE.m888adjustByBoundaryDvylE(textLayoutResult, newRawSelectionRange, new SelectionAdjustment$Companion$Word$1$adjust$1(textLayoutResult));
            }
        };
        private static final SelectionAdjustment Paragraph = new SelectionAdjustment() { // from class: androidx.compose.foundation.text.selection.SelectionAdjustment$Companion$Paragraph$1
            ...
            public long mo886adjustZXO7KMw(TextLayoutResult textLayoutResult, long newRawSelectionRange, int previousHandleOffset, boolean isStartHandle, TextRange previousSelectionRange) {
                ...
                return SelectionAdjustment.Companion.$$INSTANCE.m888adjustByBoundaryDvylE(textLayoutResult, newRawSelectionRange, (Function1) boundaryFun);
            }
        };
        private static final SelectionAdjustment CharacterWithWordAccelerate = new SelectionAdjustment() { // from class: androidx.compose.foundation.text.selection.SelectionAdjustment$Companion$CharacterWithWordAccelerate$1
            ...
            public long mo886adjustZXO7KMw(TextLayoutResult textLayoutResult, long newRawSelectionRange, int previousHandleOffset, boolean isStartHandle, TextRange previousSelectionRange) {
                ...
                if (previousSelectionRange == null) {
                    return SelectionAdjustment.Companion.$$INSTANCE.getWord().mo886adjustZXO7KMw(textLayoutResult, newRawSelectionRange, previousHandleOffset, isStartHandle, previousSelectionRange);
                }
                if (TextRange.m4715getCollapsedimpl(newRawSelectionRange)) {
                    return SelectionAdjustmentKt.ensureAtLeastOneChar(textLayoutResult.getLayoutInput().getText().getText(), TextRange.m4721getStartimpl(newRawSelectionRange), StringsKt.getLastIndex(textLayoutResult.getLayoutInput().getText()), isStartHandle, TextRange.m4720getReversedimpl(previousSelectionRange.getPackedValue()));
                }
                if (isStartHandle) {
                    start = updateSelectionBoundary(textLayoutResult, TextRange.m4721getStartimpl(newRawSelectionRange), previousHandleOffset, TextRange.m4721getStartimpl(previousSelectionRange.getPackedValue()), TextRange.m4716getEndimpl(newRawSelectionRange), true, TextRange.m4720getReversedimpl(newRawSelectionRange));
                    end = TextRange.m4716getEndimpl(newRawSelectionRange);
                } else {
                    start = TextRange.m4721getStartimpl(newRawSelectionRange);
                    end = updateSelectionBoundary(textLayoutResult, TextRange.m4716getEndimpl(newRawSelectionRange), previousHandleOffset, TextRange.m4716getEndimpl(previousSelectionRange.getPackedValue()), TextRange.m4721getStartimpl(newRawSelectionRange), false, TextRange.m4720getReversedimpl(newRawSelectionRange));
                }
            ...
            private final int snapToWordBoundary(TextLayoutResult textLayoutResult, int newRawOffset, int currentLine, int otherBoundaryOffset, boolean isStart, boolean isReversed) {
                int start;
                int end;
                long wordBoundary = textLayoutResult.m4695getWordBoundaryjx7JFs(newRawOffset);
                int wordStartLine = textLayoutResult.getLineForOffset(TextRange.m4721getStartimpl(wordBoundary));
                if (wordStartLine == currentLine) {
                    start = TextRange.m4721getStartimpl(wordBoundary);
                } else {
                    start = textLayoutResult.getLineStart(currentLine);
                }
                int wordEndLine = textLayoutResult.getLineForOffset(TextRange.m4716getEndimpl(wordBoundary));
                if (wordEndLine != currentLine) {
                    end = TextLayoutResult.getLineEnd$default(textLayoutResult, currentLine, false, 2, null);
                } else {
                    end = TextRange.m4716getEndimpl(wordBoundary);
                }
            ...
            private final boolean isAtWordBoundary(TextLayoutResult $this$isAtWordBoundary, int offset) {
                long wordBoundary = $this$isAtWordBoundary.m4695getWordBoundaryjx7JFs(offset);
                return offset == TextRange.m4721getStartimpl(wordBoundary) || offset == TextRange.m4716getEndimpl(wordBoundary);
            }
        ...
        public final long m888adjustByBoundaryDvylE(TextLayoutResult textLayoutResult, long newRawSelection, Function1<? super Integer, TextRange> boundaryFun) {
            if (textLayoutResult.getLayoutInput().getText().length() == 0) {
                return TextRange.INSTANCE.m4726getZerod9O1mEE();
            }
            int maxOffset = StringsKt.getLastIndex(textLayoutResult.getLayoutInput().getText());
            long startBoundary = boundaryFun.invoke(Integer.valueOf(RangesKt.coerceIn(TextRange.m4721getStartimpl(newRawSelection), 0, maxOffset))).getPackedValue();
            long endBoundary = boundaryFun.invoke(Integer.valueOf(RangesKt.coerceIn(TextRange.m4716getEndimpl(newRawSelection), 0, maxOffset))).getPackedValue();
            int start = TextRange.m4720getReversedimpl(newRawSelection) ? TextRange.m4716getEndimpl(startBoundary) : TextRange.m4721getStartimpl(startBoundary);
            int end = TextRange.m4720getReversedimpl(newRawSelection) ? TextRange.m4721getStartimpl(endBoundary) : TextRange.m4716getEndimpl(endBoundary);
            return TextRangeKt.TextRange(start, end);
        }
