...
public enum SelectionMode {
    Vertical { // from class: androidx.compose.foundation.text.selection.SelectionMode.Vertical
        ...
        public int mo933compare3MmeM6k$foundation_release(long position, Rect bounds) {
            Intrinsics.checkNotNullParameter(bounds, "bounds");
            if (bounds.m2734containsk4lQ0M(position)) {
                return 0;
            }
            if (Offset.m2709getYimpl(position) < bounds.getTop()) {
                return -1;
            }
            return (Offset.m2708getXimpl(position) >= bounds.getLeft() || Offset.m2709getYimpl(position) >= bounds.getBottom()) ? 1 : -1;
        }
    },
    Horizontal { // from class: androidx.compose.foundation.text.selection.SelectionMode.Horizontal
        ...
        public int mo933compare3MmeM6k$foundation_release(long position, Rect bounds) {
            ...
            if (Offset.m2708getXimpl(position) < bounds.getLeft()) {
                return -1;
            }
            return (Offset.m2709getYimpl(position) >= bounds.getTop() || Offset.m2708getXimpl(position) >= bounds.getRight()) ? 1 : -1;
        }
    };
    ...
    public abstract int mo933compare3MmeM6k$foundation_release(long position, Rect bounds);

    /* JADX INFO: renamed from: isSelected-2x9bVx0$foundation_release, reason: not valid java name */
    public final boolean m934isSelected2x9bVx0$foundation_release(Rect bounds, long start, long end) {
        Intrinsics.checkNotNullParameter(bounds, "bounds");
        if (bounds.m2734containsk4lQ0M(start) || bounds.m2734containsk4lQ0M(end)) {
            return true;
        }
        int compareStart = mo933compare3MmeM6k$foundation_release(start, bounds);
        int compareEnd = mo933compare3MmeM6k$foundation_release(end, bounds);
