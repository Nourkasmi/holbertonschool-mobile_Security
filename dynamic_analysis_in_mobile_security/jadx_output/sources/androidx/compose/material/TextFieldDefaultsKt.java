...
public final class TextFieldDefaultsKt {
    ...
    public static final State<BorderStroke> m1245animateBorderStrokeAsStateNuRrP5Q(boolean enabled, boolean isError, InteractionSource interactionSource, TextFieldColors colors, float focusedBorderThickness, float unfocusedBorderThickness, Composer $composer, int $changed) {
        ...
        if (enabled) {
            ...
            stateRememberUpdatedState = AnimateAsStateKt.m81animateDpAsStateAjpBEmI(targetThickness, AnimationSpecKt.tween$default(150, 0, null, 6, null), null, null, $composer, 48, 12);
            $composer.endReplaceableGroup();
        } else {
            $composer.startReplaceableGroup(1685712164);
            ComposerKt.sourceInformation($composer, "849@38446L46");
            stateRememberUpdatedState = SnapshotStateKt.rememberUpdatedState(Dp.m5210boximpl(unfocusedBorderThickness), $composer, ($changed >> 15) & 14);
            $composer.endReplaceableGroup();
        }
        State<BorderStroke> stateRememberUpdatedState2 = SnapshotStateKt.rememberUpdatedState(new BorderStroke(stateRememberUpdatedState.getValue().m5226unboximpl(), new SolidColor(stateIndicatorColor.getValue().m2957unboximpl(), null), null), $composer, 0);
        if (ComposerKt.isTraceInProgress()) {
