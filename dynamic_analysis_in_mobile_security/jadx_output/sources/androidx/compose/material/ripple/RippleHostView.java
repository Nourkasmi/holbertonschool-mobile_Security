...
public final class RippleHostView extends View {
    ...
    public final void m1281addRippleKOepWvA(PressInteraction.Press interaction, boolean bounded, long size, int radius, long color, float alpha, Function0<Unit> onInvalidateRipple) {
        ...
        m1282updateRipplePropertiesbiQXAtU(size, radius, color, alpha);
        if (bounded) {
            ripple.setHotspot(Offset.m2708getXimpl(interaction.getPressPosition()), Offset.m2709getYimpl(interaction.getPressPosition()));
        } else {
    ...
    public final void m1282updateRipplePropertiesbiQXAtU(long size, int radius, long color, float alpha) {
        UnprojectedRipple ripple = this.ripple;
        if (ripple == null) {
            return;
        }
        ripple.trySetRadius(radius);
        ripple.m1289setColorDxMtmZc(color, alpha);
        Rect newBounds = new Rect(0, 0, MathKt.roundToInt(Size.m2777getWidthimpl(size)), MathKt.roundToInt(Size.m2774getHeightimpl(size)));
