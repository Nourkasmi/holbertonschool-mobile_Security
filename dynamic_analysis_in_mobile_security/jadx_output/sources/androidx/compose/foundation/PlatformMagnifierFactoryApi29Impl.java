...
public final class PlatformMagnifierFactoryApi29Impl implements PlatformMagnifierFactory {
    ...
    public PlatformMagnifierImpl create(MagnifierStyle style, View view, Density density, float initialZoom) {
        ...
        if (!Intrinsics.areEqual(style, MagnifierStyle.INSTANCE.getTextDefault())) {
            long size = density.mo327toSizeXkaWNTQ(style.getSize());
            float cornerRadius = density.mo326toPx0680j_4(style.getCornerRadius());
            float elevation = density.mo326toPx0680j_4(style.getElevation());
            Magnifier.Builder $this$create_u24lambda_u241_u24lambda_u240 = new Magnifier.Builder(view);
            if (size != Size.INSTANCE.m2785getUnspecifiedNHjbRc()) {
                $this$create_u24lambda_u241_u24lambda_u240.setSize(MathKt.roundToInt(Size.m2777getWidthimpl(size)), MathKt.roundToInt(Size.m2774getHeightimpl(size)));
            }
    ...
    public static final class PlatformMagnifierImpl extends PlatformMagnifierFactoryApi28Impl.PlatformMagnifierImpl {
        ...
        public void mo243updateWko1d7g(long sourceCenter, long magnifierCenter, float zoom) {
            if (!Float.isNaN(zoom)) {
                getMagnifier().setZoom(zoom);
            }
            if (OffsetKt.m2727isSpecifiedk4lQ0M(magnifierCenter)) {
                getMagnifier().show(Offset.m2708getXimpl(sourceCenter), Offset.m2709getYimpl(sourceCenter), Offset.m2708getXimpl(magnifierCenter), Offset.m2709getYimpl(magnifierCenter));
            } else {
                getMagnifier().show(Offset.m2708getXimpl(sourceCenter), Offset.m2709getYimpl(sourceCenter));
            }
        }
    }
}
