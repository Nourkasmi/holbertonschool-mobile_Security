...
final class AndroidConfig implements ScrollConfig {
    ...
    public long mo244calculateMouseWheelScroll8xgXZGE(Density calculateMouseWheelScroll, PointerEvent event, long bounds) {
        Intrinsics.checkNotNullParameter(calculateMouseWheelScroll, "$this$calculateMouseWheelScroll");
        Intrinsics.checkNotNullParameter(event, "event");
        List<PointerInputChange> changes = event.getChanges();
        Offset offsetM2697boximpl = Offset.m2697boximpl(Offset.INSTANCE.m2724getZeroF1C5BW0());
        int index$iv$iv = 0;
        int size = changes.size();
        while (index$iv$iv < size) {
            Object item$iv$iv = changes.get(index$iv$iv);
            PointerInputChange c = (PointerInputChange) item$iv$iv;
            long acc = offsetM2697boximpl.getPackedValue();
            offsetM2697boximpl = Offset.m2697boximpl(Offset.m2713plusMKHz9U(acc, c.getScrollDelta()));
            index$iv$iv++;
            changes = changes;
        }
        return Offset.m2715timestuRUvjQ(offsetM2697boximpl.getPackedValue(), -calculateMouseWheelScroll.mo326toPx0680j_4(Dp.m5212constructorimpl(64)));
    }
}
