...
public final class RowColumnMeasurementHelper {
    ...
    public final RowColumnMeasureHelperResult m508measureWithoutPlacing_EkL_Y(MeasureScope measureScope, long constraints, int startIndex, int endIndex) {
        boolean z;
        int targetSpace;
        int weightedSpace;
        int fixedSpace;
        ...
        long constraints2 = OrientationIndependentConstraints.m450constructorimpl(constraints, this.orientation);
        int i3 = measureScope.mo320roundToPx0680j_4(this.arrangementSpacing);
        int fixedSpace2 = 0;
        ...
        while (true) {
            ...
            if (weight > 0.0f) {
                ...
                int iM5168getMaxWidthimpl = Constraints.m5168getMaxWidthimpl(constraints2);
                Placeable placeableMo4183measureBRTryo0 = this.placeables[i4];
                if (placeableMo4183measureBRTryo0 == null) {
                    int weightChildrenCount2 = iM5168getMaxWidthimpl != Integer.MAX_VALUE ? iM5168getMaxWidthimpl - fixedSpace2 : Integer.MAX_VALUE;
                    mainAxisMax = iM5168getMaxWidthimpl;
                    z3 = false;
                    placeableMo4183measureBRTryo0 = child.mo4183measureBRTryo0(OrientationIndependentConstraints.m463toBoxConstraintsOenEA2s(OrientationIndependentConstraints.m451copyyUG9Ft0(constraints2, (8 & 1) != 0 ? Constraints.m5170getMinWidthimpl(constraints2) : 0, (8 & 2) != 0 ? Constraints.m5168getMaxWidthimpl(constraints2) : weightChildrenCount2, (8 & 4) != 0 ? Constraints.m5169getMinHeightimpl(constraints2) : 0, (8 & 8) != 0 ? Constraints.m5167getMaxHeightimpl(constraints2) : 0), this.orientation));
                } else {
                    mainAxisMax = iM5168getMaxWidthimpl;
                    z3 = false;
                }
                Placeable placeable = placeableMo4183measureBRTryo0;
                int spaceAfterLastNoWeight2 = Math.min(i3, (mainAxisMax - fixedSpace2) - mainAxisSize(placeable));
                fixedSpace2 += mainAxisSize(placeable) + spaceAfterLastNoWeight2;
                crossAxisSpace2 = Math.max(crossAxisSpace2, crossAxisSize(placeable));
        ...
        if (weightChildrenCount == 0) {
            ...
            if (totalWeight > 0.0f && Constraints.m5168getMaxWidthimpl(constraints2) != Integer.MAX_VALUE) {
                targetSpace = Constraints.m5168getMaxWidthimpl(constraints2);
            } else {
                targetSpace = Constraints.m5170getMinWidthimpl(constraints2);
            }
            ...
            int iRoundToInt = 0;
            while (it.hasNext()) {
                iRoundToInt += MathKt.roundToInt(RowColumnImplKt.getWeight(this.rowColumnParentData[((IntIterator) it).nextInt()]) * weightUnitSpace2);
            }
            int childMainAxisSize = remainingToTarget2 - iRoundToInt;
            int i5 = startIndex;
            while (i5 < i2) {
                if (this.placeables[i5] != null) {
                    targetSpace2 = targetSpace;
                    z2 = z;
                    remainingToTarget = remainingToTarget2;
                    weightUnitSpace = weightUnitSpace2;
                } else {
                    Measurable child2 = this.measurables.get(i5);
                    RowColumnParentData parentData2 = this.rowColumnParentData[i5];
                    ...
                    remainingToTarget = remainingToTarget2;
                    weightUnitSpace = weightUnitSpace2;
                    Placeable placeable2 = child2.mo4183measureBRTryo0(OrientationIndependentConstraints.m463toBoxConstraintsOenEA2s(OrientationIndependentConstraints.m448constructorimpl(i, childMainAxisSize2, 0, Constraints.m5167getMaxHeightimpl(constraints2)), this.orientation));
                    weightedSpace2 += mainAxisSize(placeable2);
            ...
            weightedSpace = RangesKt.coerceAtMost(((weightChildrenCount - 1) * i3) + weightedSpace2, Constraints.m5168getMaxWidthimpl(constraints2) - fixedSpace2);
        ...
        if (!anyAlignBy) {
            ...
            for (int i6 = startIndex; i6 < endIndex; i6++) {
                Placeable placeable3 = this.placeables[i6];
                Intrinsics.checkNotNull(placeable3);
                CrossAxisAlignment crossAxisAlignment = RowColumnImplKt.getCrossAxisAlignment(this.rowColumnParentData[i6]);
                ...
                if (alignmentLinePosition != null) {
                    ...
                    int iCrossAxisSize2 = crossAxisSize(placeable3);
                    int it3 = alignmentLinePosition.intValue();
                    if (it3 != Integer.MIN_VALUE) {
                        iCrossAxisSize = it3;
                    } else {
                        iCrossAxisSize = crossAxisSize(placeable3);
                    }
                    afterCrossAxisAlignmentLine2 = Math.max(afterCrossAxisAlignmentLine2, iCrossAxisSize2 - iCrossAxisSize);
        ...
        int mainAxisLayoutSize = Math.max(beforeCrossAxisAlignmentLine4, Constraints.m5170getMinWidthimpl(constraints2));
        if (Constraints.m5167getMaxHeightimpl(constraints2) == Integer.MAX_VALUE || this.crossAxisSize != SizeMode.Expand) {
            crossAxisLayoutSize = Math.max(crossAxisSpace, Math.max(Constraints.m5169getMinHeightimpl(constraints2), beforeCrossAxisAlignmentLine + afterCrossAxisAlignmentLine));
        } else {
            crossAxisLayoutSize = Constraints.m5167getMaxHeightimpl(constraints2);
        }
        int[] mainAxisPositions = new int[subSize];
        for (int i7 = 0; i7 < subSize; i7++) {
            mainAxisPositions[i7] = 0;
        }
        int i8 = 0;
        int[] childrenMainAxisSize = new int[subSize];
        while (i8 < subSize) {
            int subSize2 = subSize;
            Placeable placeable4 = this.placeables[i8 + startIndex];
            Intrinsics.checkNotNull(placeable4);
            childrenMainAxisSize[i8] = mainAxisSize(placeable4);
            i8++;
            subSize = subSize2;
        }
