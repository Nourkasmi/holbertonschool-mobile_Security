...
public interface GridCells {
    ...
    public static final class Adaptive implements GridCells {
        ...
        private Adaptive(float minSize) {
            ...
            int $this$dp$iv = Dp.m5211compareTo0680j_4(minSize, Dp.m5212constructorimpl(0));
            if ($this$dp$iv > 0) {
            } else {
                throw new IllegalArgumentException(("Provided min size " + ((Object) Dp.m5223toStringimpl(minSize)) + " should be larger than zero.").toString());
            }
        }
        ...
        public List<Integer> calculateCrossAxisCellSizes(Density $this$calculateCrossAxisCellSizes, int availableSize, int spacing) {
            Intrinsics.checkNotNullParameter($this$calculateCrossAxisCellSizes, "<this>");
            int count = Math.max((availableSize + spacing) / ($this$calculateCrossAxisCellSizes.mo320roundToPx0680j_4(this.minSize) + spacing), 1);
            return LazyGridDslKt.calculateCellsCrossAxisSizeImpl(availableSize, count, spacing);
        }

        public int hashCode() {
            return Dp.m5218hashCodeimpl(this.minSize);
        }

        public boolean equals(Object other) {
            return (other instanceof Adaptive) && Dp.m5217equalsimpl0(this.minSize, ((Adaptive) other).minSize);
        }
    }
    ...
    public static final class FixedSize implements GridCells {
        ...
        private FixedSize(float size) {
            this.size = size;
            int $this$dp$iv = Dp.m5211compareTo0680j_4(size, Dp.m5212constructorimpl(0));
            if ($this$dp$iv > 0) {
            } else {
                throw new IllegalArgumentException(("Provided size " + ((Object) Dp.m5223toStringimpl(size)) + " should be larger than zero.").toString());
            }
        }
        ...
        public List<Integer> calculateCrossAxisCellSizes(Density $this$calculateCrossAxisCellSizes, int availableSize, int spacing) {
            Intrinsics.checkNotNullParameter($this$calculateCrossAxisCellSizes, "<this>");
            int cellSize = $this$calculateCrossAxisCellSizes.mo320roundToPx0680j_4(this.size);
            int i = 0;
        ...
        public int hashCode() {
            return Dp.m5218hashCodeimpl(this.size);
        }

        public boolean equals(Object other) {
            return (other instanceof FixedSize) && Dp.m5217equalsimpl0(this.size, ((FixedSize) other).size);
        }
    }
}
