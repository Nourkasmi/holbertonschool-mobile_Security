...
public abstract class LazyGridMeasuredLineProvider {
    ...
    public final long m621childConstraintsJhjzzOo$foundation_release(int startSlot, int span) {
        int i;
        if (span != 1) {
        ...
        if (this.isVertical) {
            return Constraints.INSTANCE.m5178fixedWidthOenEA2s(crossAxisSize);
        }
        return Constraints.INSTANCE.m5177fixedHeightOenEA2s(crossAxisSize);
    }

    /* JADX INFO: renamed from: itemConstraints-OenEA2s, reason: not valid java name */
    public final long m622itemConstraintsOenEA2s(int itemIndex) {
        ...
        return m621childConstraintsJhjzzOo$foundation_release(0, span);
    }
    ...
    public final LazyGridMeasuredLine getAndMeasure(int lineIndex) {
        ...
        for (int i = 0; i < lineItemsCount; i++) {
            int span = GridItemSpan.m600getCurrentLineSpanimpl(lineConfiguration.getSpans().get(i).getPackedValue());
            long constraints = m621childConstraintsJhjzzOo$foundation_release(startSlot, span);
            LazyGridMeasuredItem lazyGridMeasuredItemM620getAndMeasure3p2s80s = this.measuredItemProvider.m620getAndMeasure3p2s80s(lineConfiguration.getFirstItemIndex() + i, mainAxisSpacing, constraints);
            startSlot += span;
            Unit unit = Unit.INSTANCE;
            items[i] = lazyGridMeasuredItemM620getAndMeasure3p2s80s;
        }
