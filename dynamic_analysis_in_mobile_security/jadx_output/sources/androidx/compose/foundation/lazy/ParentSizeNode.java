...
final class ParentSizeNode extends Modifier.Node implements LayoutModifierNode {
    ...
    public MeasureResult mo238measure3p2s80s(MeasureScope measure, Measurable measurable, long constraints) {
        ...
        if (state != null && state.getValue().intValue() != Integer.MAX_VALUE) {
            iRoundToInt = MathKt.roundToInt(state.getValue().floatValue() * this.fraction);
        } else {
            iRoundToInt = Integer.MAX_VALUE;
        }
        int width = iRoundToInt;
        State<Integer> state2 = this.heightState;
        if (state2 != null && state2.getValue().intValue() != Integer.MAX_VALUE) {
            iRoundToInt2 = MathKt.roundToInt(state2.getValue().floatValue() * this.fraction);
        } else {
            iRoundToInt2 = Integer.MAX_VALUE;
        }
        int height = iRoundToInt2;
        long childConstraints = ConstraintsKt.Constraints(width != Integer.MAX_VALUE ? width : Constraints.m5170getMinWidthimpl(constraints), width != Integer.MAX_VALUE ? width : Constraints.m5168getMaxWidthimpl(constraints), height != Integer.MAX_VALUE ? height : Constraints.m5169getMinHeightimpl(constraints), height != Integer.MAX_VALUE ? height : Constraints.m5167getMaxHeightimpl(constraints));
