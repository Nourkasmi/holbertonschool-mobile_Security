...
public final class LazyStaggeredGridAnimateScrollScope implements LazyAnimateScrollScope {
    ...
    public Integer getTargetItemOffset(int index) {
        ...
        long it = lazyStaggeredGridItemInfoFindVisibleItem.getOffset();
        return Integer.valueOf(this.state.getIsVertical() ? IntOffset.m5331getYimpl(it) : IntOffset.m5330getXimpl(it));
    }
    ...
    public float expectedDistanceTo(int index, int targetScrollOffset) {
        ...
        for (int index$iv$iv = 0; index$iv$iv < size; index$iv$iv++) {
            ...
            sum$iv += this.state.getIsVertical() ? IntSize.m5371getHeightimpl(it.getSize()) : IntSize.m5372getWidthimpl(it.getSize());
        }
        int itemSizeSum = sum$iv;
        int averageMainAxisItemSize = (itemSizeSum / visibleItemsInfo.size()) + layoutInfo.getMainAxisItemSpacing();
