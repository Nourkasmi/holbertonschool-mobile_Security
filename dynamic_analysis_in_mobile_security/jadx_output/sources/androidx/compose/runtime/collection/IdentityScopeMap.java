...
public final class IdentityScopeMap<T> {
    ...
    private final IdentityArraySet<T> getOrCreateIdentitySet(Object value) {
        ...
        int[] newKeyOrder;
        Object[] newValues;
        IdentityArraySet<T>[] identityArraySetArr;
        int size = this.size;
        int[] valueOrder = this.valueOrder;
        Object[] values = this.values;
        IdentityArraySet<T>[] identityArraySetArr2 = this.scopeSets;
        if (size > 0) {
        ...
        if (size < valueOrder.length) {
            ...
            IdentityArraySet<T> identityArraySet = identityArraySetArr2[valueIndex];
            if (identityArraySet == null) {
                identityArraySet = new IdentityArraySet<>();
                identityArraySetArr2[valueIndex] = identityArraySet;
            }
            if (insertIndex < size) {
        ...
        Object[] objArrCopyOf = Arrays.copyOf(identityArraySetArr2, newSize);
        Intrinsics.checkNotNullExpressionValue(objArrCopyOf, "copyOf(this, newSize)");
        IdentityArraySet<T>[] identityArraySetArr3 = (IdentityArraySet[]) objArrCopyOf;
        IdentityArraySet<T> identityArraySet2 = new IdentityArraySet<>();
        identityArraySetArr3[size] = identityArraySet2;
    ...
    public final boolean remove(Object value, T scope) {
        ...
        Object[] values = this.values;
        int size = this.size;
        if (index < 0 || (identityArraySet = identityArraySetArr[(valueOrderIndex = valueOrder[index])]) == null) {
            return false;
        }
        boolean removed = identityArraySet.remove(scope);
    ...
    public final void removeValueIf(Function1<? super T, Boolean> predicate) {
        ...
        while (i$iv < size) {
            ...
            IdentityArraySet<T> identityArraySet = scopeSets[valueIndex$iv];
            Intrinsics.checkNotNull(identityArraySet);
            Object[] values$iv2 = identityArraySet.getValues();
            int size$iv = identityArraySet.size();
            ...
            while ($i$f$removingScopes2 < size$iv) {
                IdentityArraySet<T>[] identityArraySetArr = scopeSets;
            ...
            IdentityArraySet<T>[] identityArraySetArr2 = scopeSets;
            int i2 = size;
            for (int i$iv2 = destinationIndex$iv2; i$iv2 < size$iv; i$iv2++) {
                values$iv2[i$iv2] = null;
            }
            ((IdentityArraySet) identityArraySet).size = destinationIndex$iv2;
            if (identityArraySet.size() > 0) {
