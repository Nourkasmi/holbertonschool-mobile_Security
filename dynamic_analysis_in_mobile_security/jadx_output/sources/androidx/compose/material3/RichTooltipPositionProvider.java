...
final /* data */ class RichTooltipPositionProvider implements PopupPositionProvider {
    ...
    public long mo875calculatePositionllwVHH4(IntRect anchorBounds, long windowSize, LayoutDirection layoutDirection, long popupContentSize) {
        ...
        if (IntSize.m5372getWidthimpl(popupContentSize) + x > IntSize.m5372getWidthimpl(windowSize) && (x = anchorBounds.getLeft() - IntSize.m5372getWidthimpl(popupContentSize)) < 0) {
            x = anchorBounds.getLeft() + ((anchorBounds.getWidth() - IntSize.m5372getWidthimpl(popupContentSize)) / 2);
        }
        int y = (anchorBounds.getTop() - IntSize.m5371getHeightimpl(popupContentSize)) - this.tooltipAnchorPadding;
        if (y < 0) {
