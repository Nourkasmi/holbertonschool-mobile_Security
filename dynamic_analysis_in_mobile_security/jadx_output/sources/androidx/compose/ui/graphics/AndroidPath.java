...
public final class AndroidPath implements Path {
    ...
    public int mo2840getFillTypeRgk1Os() {
        if (this.internalPath.getFillType() == Path.FillType.EVEN_ODD) {
            return PathFillType.INSTANCE.m3222getEvenOddRgk1Os();
        }
        return PathFillType.INSTANCE.m3223getNonZeroRgk1Os();
    }

    @Override // androidx.compose.ui.graphics.Path
    /* JADX INFO: renamed from: setFillType-oQ8Xj4U, reason: not valid java name */
    public void mo2842setFillTypeoQ8Xj4U(int value) {
        Path.FillType fillType;
        android.graphics.Path path = this.internalPath;
        if (PathFillType.m3218equalsimpl0(value, PathFillType.INSTANCE.m3222getEvenOddRgk1Os())) {
    ...
    public void addRoundRect(RoundRect roundRect) {
        ...
        this.radii[0] = CornerRadius.m2683getXimpl(roundRect.m2758getTopLeftCornerRadiuskKHJgLs());
        this.radii[1] = CornerRadius.m2684getYimpl(roundRect.m2758getTopLeftCornerRadiuskKHJgLs());
        this.radii[2] = CornerRadius.m2683getXimpl(roundRect.m2759getTopRightCornerRadiuskKHJgLs());
        this.radii[3] = CornerRadius.m2684getYimpl(roundRect.m2759getTopRightCornerRadiuskKHJgLs());
        this.radii[4] = CornerRadius.m2683getXimpl(roundRect.m2757getBottomRightCornerRadiuskKHJgLs());
        this.radii[5] = CornerRadius.m2684getYimpl(roundRect.m2757getBottomRightCornerRadiuskKHJgLs());
        this.radii[6] = CornerRadius.m2683getXimpl(roundRect.m2756getBottomLeftCornerRadiuskKHJgLs());
        this.radii[7] = CornerRadius.m2684getYimpl(roundRect.m2756getBottomLeftCornerRadiuskKHJgLs());
    ...
    public void mo2839addPathUv8p0NA(Path path, long offset) {
        Intrinsics.checkNotNullParameter(path, "path");
        android.graphics.Path path2 = this.internalPath;
        if (path instanceof AndroidPath) {
            path2.addPath(((AndroidPath) path).getInternalPath(), Offset.m2708getXimpl(offset), Offset.m2709getYimpl(offset));
            return;
        }
    ...
    public void mo2844translatek4lQ0M(long offset) {
        this.mMatrix.reset();
        this.mMatrix.setTranslate(Offset.m2708getXimpl(offset), Offset.m2709getYimpl(offset));
        this.internalPath.transform(this.mMatrix);
    }

    @Override // androidx.compose.ui.graphics.Path
    /* JADX INFO: renamed from: transform-58bKbWc, reason: not valid java name */
    public void mo2843transform58bKbWc(float[] matrix) {
        Intrinsics.checkNotNullParameter(matrix, "matrix");
        AndroidMatrixConversions_androidKt.m2819setFromEL8BTi8(this.mMatrix, matrix);
        this.internalPath.transform(this.mMatrix);
    }
    ...
    public boolean mo2841opN5in7k0(Path path1, Path path2, int operation) {
        Path.Op op;
        Intrinsics.checkNotNullParameter(path1, "path1");
        Intrinsics.checkNotNullParameter(path2, "path2");
        if (PathOperation.m3227equalsimpl0(operation, PathOperation.INSTANCE.m3231getDifferenceb3I0S0c())) {
            op = Path.Op.DIFFERENCE;
        } else if (PathOperation.m3227equalsimpl0(operation, PathOperation.INSTANCE.m3232getIntersectb3I0S0c())) {
            op = Path.Op.INTERSECT;
        } else if (PathOperation.m3227equalsimpl0(operation, PathOperation.INSTANCE.m3233getReverseDifferenceb3I0S0c())) {
            op = Path.Op.REVERSE_DIFFERENCE;
        } else {
            op = PathOperation.m3227equalsimpl0(operation, PathOperation.INSTANCE.m3234getUnionb3I0S0c()) ? Path.Op.UNION : Path.Op.XOR;
        }
        android.graphics.Path path = this.internalPath;
