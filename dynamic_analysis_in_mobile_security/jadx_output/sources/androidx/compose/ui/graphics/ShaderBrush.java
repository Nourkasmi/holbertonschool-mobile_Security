...
public abstract class ShaderBrush extends Brush {
    ...
    public abstract Shader mo2916createShaderuvyYCjk(long size);

    public ShaderBrush() {
        super(null);
        this.createdSize = Size.INSTANCE.m2785getUnspecifiedNHjbRc();
    }

    @Override // androidx.compose.ui.graphics.Brush
    /* JADX INFO: renamed from: applyTo-Pq9zytI */
    public final void mo2894applyToPq9zytI(long size, Paint p, float alpha) {
        Intrinsics.checkNotNullParameter(p, "p");
        Shader shader = this.internalShader;
        if (shader == null || !Size.m2773equalsimpl0(this.createdSize, size)) {
            if (Size.m2779isEmptyimpl(size)) {
                shader = null;
                this.internalShader = null;
                this.createdSize = Size.INSTANCE.m2785getUnspecifiedNHjbRc();
            } else {
                Shader it = mo2916createShaderuvyYCjk(size);
                this.internalShader = it;
                shader = it;
                this.createdSize = size;
            }
        }
        if (!Color.m2948equalsimpl0(p.mo2822getColor0d7_KjU(), Color.INSTANCE.m2973getBlack0d7_KjU())) {
            p.mo2828setColor8_81llA(Color.INSTANCE.m2973getBlack0d7_KjU());
        }
