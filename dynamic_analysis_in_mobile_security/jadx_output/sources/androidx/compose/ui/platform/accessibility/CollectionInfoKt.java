    ...
    public static final void setCollectionItemInfo(SemanticsNode node, AccessibilityNodeInfoCompat info) {
        ...
        if (parentNode != null && SemanticsConfigurationKt.getOrNull(parentNode.getConfig(), SemanticsProperties.INSTANCE.getSelectableGroup()) != null) {
            ...
            if ((collectionInfo == null || !isLazyCollection(collectionInfo)) && node.getConfig().contains(SemanticsProperties.INSTANCE.getSelected())) {
                List groupedChildren = new ArrayList();
                int index = 0;
                List<SemanticsNode> replacedChildren$ui_release = parentNode.getReplacedChildren$ui_release();
                int size = replacedChildren$ui_release.size();
                for (int index$iv = 0; index$iv < size; index$iv++) {
                    Object item$iv = replacedChildren$ui_release.get(index$iv);
                    SemanticsNode childNode = (SemanticsNode) item$iv;
    ...
    private static final boolean calculateIfHorizontallyStacked(List<SemanticsNode> list) {
        ...
        if (list.size() == 0 || list.size() == 1) {
            ...
            for (int i$iv = 0; i$iv < lastIndex; i$iv++) {
                ...
                result$iv.add(Offset.m2697boximpl(OffsetKt.Offset(Math.abs(Offset.m2708getXimpl(el1.getBoundsInRoot().m2738getCenterF1C5BW0()) - Offset.m2708getXimpl(el2.getBoundsInRoot().m2738getCenterF1C5BW0())), Math.abs(Offset.m2709getYimpl(el1.getBoundsInRoot().m2738getCenterF1C5BW0()) - Offset.m2709getYimpl(el2.getBoundsInRoot().m2738getCenterF1C5BW0())))));
                current$iv = next$iv;
            }
        }
        ...
        if ($this$fastZipWithNext$iv.size() == 1) {
            ...
            if (1 <= lastIndex2) {
                while (true) {
                    ...
                    accumulator$iv = Offset.m2697boximpl(Offset.m2713plusMKHz9U(result, element));
                    if (i$iv2 == lastIndex2) {
                        break;
                    }
                    i$iv2++;
        ...
        float deltaX = Offset.m2698component1impl(packedValue);
        float deltaY = Offset.m2699component2impl(packedValue);
