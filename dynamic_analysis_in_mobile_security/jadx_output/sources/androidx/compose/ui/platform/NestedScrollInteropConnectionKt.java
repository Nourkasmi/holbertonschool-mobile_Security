...
public final class NestedScrollInteropConnectionKt {
    ...
    public static final long m4534toOffsetUv8p0NA(int[] consumed, long available) {
        float offsetX;
        float offsetY;
        if (Offset.m2708getXimpl(available) >= 0.0f) {
            offsetX = RangesKt.coerceAtMost(reverseAxis(consumed[0]), Offset.m2708getXimpl(available));
        } else {
            offsetX = RangesKt.coerceAtLeast(reverseAxis(consumed[0]), Offset.m2708getXimpl(available));
        }
        if (Offset.m2709getYimpl(available) >= 0.0f) {
            offsetY = RangesKt.coerceAtMost(reverseAxis(consumed[1]), Offset.m2709getYimpl(available));
        } else {
            offsetY = RangesKt.coerceAtLeast(reverseAxis(consumed[1]), Offset.m2709getYimpl(available));
        }
    ...
    public static final int m4535toViewTypeGyEprt8(int $this$toViewType_u2dGyEprt8) {
        return NestedScrollSource.m3957equalsimpl0($this$toViewType_u2dGyEprt8, NestedScrollSource.INSTANCE.m3962getDragWNlRxjI()) ? 0 : 1;
    }

    /* JADX INFO: Access modifiers changed from: private */
    /* JADX INFO: renamed from: getScrollAxes-k-4lQ0M, reason: not valid java name */
    public static final int m4533getScrollAxesk4lQ0M(long $this$scrollAxes) {
        int axes = 0;
        if (Math.abs(Offset.m2708getXimpl($this$scrollAxes)) >= 0.5f) {
            axes = 0 | 1;
        }
        if (Math.abs(Offset.m2709getYimpl($this$scrollAxes)) >= 0.5f) {
            return axes | 2;
        }
        return axes;
    }
