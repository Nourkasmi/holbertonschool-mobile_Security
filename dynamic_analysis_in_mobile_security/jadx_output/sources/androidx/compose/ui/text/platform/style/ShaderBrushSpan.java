...
public final class ShaderBrushSpan extends CharacterStyle implements UpdateAppearance {
    ...
    public ShaderBrushSpan(ShaderBrush shaderBrush, float alpha) {
        ...
        this.alpha = alpha;
        this.size = Size.INSTANCE.m2785getUnspecifiedNHjbRc();
    }
    ...
    public final void m4976setSizeuvyYCjk(long j) {
        this.size = j;
    }
    ...
    public void updateDrawState(TextPaint textPaint) {
        ...
        if ($this$isUnspecified$iv == Size.INSTANCE.m2785getUnspecifiedNHjbRc()) {
            return;
        }
        Pair<Size, ? extends Shader> pair = this.cachedShader;
        if (pair == null || !Size.m2773equalsimpl0(pair.getFirst().getPackedValue(), this.size)) {
            shader = this.shaderBrush.mo2916createShaderuvyYCjk(this.size);
        } else {
            shader = pair.getSecond();
        }
        textPaint.setShader(shader);
        this.cachedShader = TuplesKt.to(Size.m2765boximpl(this.size), shader);
    }
}
