...
public final class TestTimeSource extends AbstractLongTimeSource {
    ...
    public final void m6960plusAssignLRDsOJo(long duration) {
        long longDelta = Duration.m6870toLongimpl(duration, getUnit());
        ...
        if ($i$f$isSaturated == 0) {
            ...
            if ((j ^ longDelta) >= 0 && (j ^ newReading) < 0) {
                m6959overflowLRDsOJo(duration);
            }
            this.reading = newReading;
            return;
        }
        long half = Duration.m6827divUwyO8pc(duration, 2);
        long $this$isSaturated$iv = Duration.m6870toLongimpl(half, getUnit());
        if (!((($this$isSaturated$iv - 1) | 1) == Long.MAX_VALUE)) {
            long readingBefore = this.reading;
            try {
                m6960plusAssignLRDsOJo(half);
                m6960plusAssignLRDsOJo(Duration.m6859minusLRDsOJo(duration, half));
                return;
    ...
    private final void m6959overflowLRDsOJo(long duration) {
        throw new IllegalStateException("TestTimeSource will overflow if its reading " + this.reading + DurationUnitKt.shortName(getUnit()) + " is advanced by " + ((Object) Duration.m6873toStringimpl(duration)) + '.');
    }
}
