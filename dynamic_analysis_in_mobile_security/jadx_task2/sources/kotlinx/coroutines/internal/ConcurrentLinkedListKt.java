    ...
    public static final Object findSegmentAndMoveForward$atomicfu(Object obj, AtomicReferenceFieldUpdater atomicReferenceFieldUpdater, long j, Segment segment, Function2 function2) {
        ...
        loop0: while (true) {
            ...
            if (!SegmentOrClosed.m6727isClosedimpl(objFindSegmentInternal)) {
                ...
                while (true) {
                    Segment segment2 = (Segment) atomicReferenceFieldUpdater.get(obj);
                    if (segment2.id >= segmentM6725getSegmentimpl.id) {
                        break loop0;
                    }
                    if (!segmentM6725getSegmentimpl.tryIncPointers$kotlinx_coroutines_core()) {
                        break;
                    }
                    if (AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(atomicReferenceFieldUpdater, obj, segment2, segmentM6725getSegmentimpl)) {
                        if (segment2.decPointers$kotlinx_coroutines_core()) {
                            segment2.remove();
                        }
        ...
        loop0: while (true) {
            ...
            if (!SegmentOrClosed.m6727isClosedimpl(objFindSegmentInternal)) {
                ...
                while (true) {
                    ...
                    if (ChannelSegment$$ExternalSyntheticBackportWithForwarding0.m(atomicReferenceArray, i, segment2, segmentM6725getSegmentimpl)) {
                        if (segment2.decPointers$kotlinx_coroutines_core()) {
                            segment2.remove();
                        }
                    } else if (segmentM6725getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
