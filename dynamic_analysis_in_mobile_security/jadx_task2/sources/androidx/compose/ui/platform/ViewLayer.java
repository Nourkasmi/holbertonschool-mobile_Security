...
public final class ViewLayer extends View implements OwnedLayer, GraphicLayerInfo {
    ...
    public boolean mo4127isInLayerk4lQ0M(long position) {
        ...
        if (this.clipToBounds) {
            return 0.0f <= fM2390getXimpl && fM2390getXimpl < ((float) getWidth()) && 0.0f <= fM2391getYimpl && fM2391getYimpl < ((float) getHeight());
        }
        if (getClipToOutline()) {
    ...
    public void mo4130resizeozmzZPI(long size) {
        ...
        if (iM5033getWidthimpl == getWidth() && iM5032getHeightimpl == getHeight()) {
            return;
        }
        ...
        updateOutlineResolver();
        layout(getLeft(), getTop(), getLeft() + iM5033getWidthimpl, getTop() + iM5032getHeightimpl);
        resetClipBounds();
        this.matrixCache.invalidate();
    }
    ...
    public void mo4129movegyyYBs(long position) {
        ...
        if (iM4991getXimpl != getLeft()) {
            offsetLeftAndRight(iM4991getXimpl - getLeft());
            this.matrixCache.invalidate();
        }
        int iM4992getYimpl = IntOffset.m4992getYimpl(position);
        if (iM4992getYimpl != getTop()) {
            offsetTopAndBottom(iM4992getYimpl - getTop());
            this.matrixCache.invalidate();
        }
    }
    ...
    public long mo4128mapOffset8S9VItk(long point, boolean inverse) {
        if (inverse) {
            float[] fArrM4197calculateInverseMatrixbWbORWo = this.matrixCache.m4197calculateInverseMatrixbWbORWo(this);
