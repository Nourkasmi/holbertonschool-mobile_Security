...
public final class NestedScrollInteropConnectionKt {
    ...
    public static final long m4217toOffsetUv8p0NA(int[] iArr, long j) {
        ...
        if (Offset.m2390getXimpl(j) >= 0.0f) {
            fCoerceAtLeast = RangesKt.coerceAtMost(reverseAxis(iArr[0]), Offset.m2390getXimpl(j));
        } else {
            fCoerceAtLeast = RangesKt.coerceAtLeast(reverseAxis(iArr[0]), Offset.m2390getXimpl(j));
        }
        if (Offset.m2391getYimpl(j) >= 0.0f) {
            fCoerceAtLeast2 = RangesKt.coerceAtMost(reverseAxis(iArr[1]), Offset.m2391getYimpl(j));
        } else {
            fCoerceAtLeast2 = RangesKt.coerceAtLeast(reverseAxis(iArr[1]), Offset.m2391getYimpl(j));
        }
