...
public final class EditingBuffer {
    ...
    private EditingBuffer(AnnotatedString text, long j) {
        ...
        this.compositionStart = -1;
        this.compositionEnd = -1;
        int iM4402getMinimpl = TextRange.m4402getMinimpl(j);
        int iM4401getMaximpl = TextRange.m4401getMaximpl(j);
        if (iM4402getMinimpl < 0 || iM4402getMinimpl > text.length()) {
            throw new IndexOutOfBoundsException("start (" + iM4402getMinimpl + ") offset is outside of text region " + text.length());
        }
        if (iM4401getMaximpl < 0 || iM4401getMaximpl > text.length()) {
            throw new IndexOutOfBoundsException("end (" + iM4401getMaximpl + ") offset is outside of text region " + text.length());
        }
        if (iM4402getMinimpl > iM4401getMaximpl) {
            throw new IllegalArgumentException("Do not set reversed range: " + iM4402getMinimpl + " > " + iM4401getMaximpl);
        }
    }
    ...
    public final void delete$ui_text_release(int start, int end) {
        ...
        long jM4547updateRangeAfterDeletepWDy79M = EditingBufferKt.m4547updateRangeAfterDeletepWDy79M(TextRangeKt.TextRange(this.selectionStart, this.selectionEnd), jTextRange);
        setSelectionStart(TextRange.m4402getMinimpl(jM4547updateRangeAfterDeletepWDy79M));
        setSelectionEnd(TextRange.m4401getMaximpl(jM4547updateRangeAfterDeletepWDy79M));
        if (hasComposition$ui_text_release()) {
            long jM4547updateRangeAfterDeletepWDy79M2 = EditingBufferKt.m4547updateRangeAfterDeletepWDy79M(TextRangeKt.TextRange(this.compositionStart, this.compositionEnd), jTextRange);
            if (TextRange.m4398getCollapsedimpl(jM4547updateRangeAfterDeletepWDy79M2)) {
                commitComposition$ui_text_release();
            } else {
                this.compositionStart = TextRange.m4402getMinimpl(jM4547updateRangeAfterDeletepWDy79M2);
                this.compositionEnd = TextRange.m4401getMaximpl(jM4547updateRangeAfterDeletepWDy79M2);
            }
        }
    }
