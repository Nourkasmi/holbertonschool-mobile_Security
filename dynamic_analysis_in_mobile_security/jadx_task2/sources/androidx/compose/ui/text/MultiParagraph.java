...
public final class MultiParagraph {
    ...
    private MultiParagraph(MultiParagraphIntrinsics intrinsics, long j, int i, boolean z) {
        ...
        while (i4 < size) {
            ...
            if (Constraints.m4824getHasBoundedHeightimpl(j)) {
                iM4828getMaxHeightimpl = RangesKt.coerceAtLeast(Constraints.m4828getMaxHeightimpl(j) - ParagraphKt.ceilToInt(f), i2);
            } else {
                iM4828getMaxHeightimpl = Constraints.m4828getMaxHeightimpl(j);
            }
            Paragraph paragraphM4304Paragraph_EkL_Y = ParagraphKt.m4304Paragraph_EkL_Y(intrinsics2, ConstraintsKt.Constraints$default(0, iM4829getMaxWidthimpl, 0, iM4828getMaxHeightimpl, 5, null), this.maxLines - i3, z);
            float height = f + paragraphM4304Paragraph_EkL_Y.getHeight();
            int lineCount = i3 + paragraphM4304Paragraph_EkL_Y.getLineCount();
            List<ParagraphIntrinsicInfo> list = infoList$ui_text_release;
            arrayList.add(new ParagraphInfo(paragraphM4304Paragraph_EkL_Y, paragraphIntrinsicInfo.getStartIndex(), paragraphIntrinsicInfo.getEndIndex(), i3, lineCount, f, height));
            if (paragraphM4304Paragraph_EkL_Y.getDidExceedMaxLines() || (lineCount == this.maxLines && i4 != CollectionsKt.getLastIndex(this.intrinsics.getInfoList$ui_text_release()))) {
                z2 = true;
                i3 = lineCount;
                f = height;
                break;
    ...
    public final int getLineForVerticalPosition(float vertical) {
        int iFindParagraphByY;
        if (vertical <= 0.0f) {
            iFindParagraphByY = 0;
        } else if (vertical >= this.height) {
            iFindParagraphByY = CollectionsKt.getLastIndex(this.paragraphInfoList);
        } else {
            iFindParagraphByY = MultiParagraphKt.findParagraphByY(this.paragraphInfoList, vertical);
        }
