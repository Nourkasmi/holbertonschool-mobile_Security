    ...
    public Placeable mo3866measureBRTryo0(long constraints) {
        ...
        if (this.widthHeight == IntrinsicWidthHeight.Width) {
            if (this.minMax == IntrinsicMinMax.Max) {
                iMinIntrinsicWidth = this.measurable.maxIntrinsicWidth(Constraints.m4828getMaxHeightimpl(constraints));
            } else {
                iMinIntrinsicWidth = this.measurable.minIntrinsicWidth(Constraints.m4828getMaxHeightimpl(constraints));
            }
            return new FixedSizeIntrinsicsPlaceable(iMinIntrinsicWidth, Constraints.m4828getMaxHeightimpl(constraints));
        }
        if (this.minMax == IntrinsicMinMax.Max) {
            iMinIntrinsicHeight = this.measurable.maxIntrinsicHeight(Constraints.m4829getMaxWidthimpl(constraints));
        } else {
            iMinIntrinsicHeight = this.measurable.minIntrinsicHeight(Constraints.m4829getMaxWidthimpl(constraints));
        }
        return new FixedSizeIntrinsicsPlaceable(Constraints.m4829getMaxWidthimpl(constraints), iMinIntrinsicHeight);
    }
