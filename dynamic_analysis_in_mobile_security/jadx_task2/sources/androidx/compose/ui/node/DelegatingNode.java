...
public abstract class DelegatingNode extends Modifier.Node {
    ...
    protected final <T extends DelegatableNode> T delegate(T delegatableNode) {
        ...
        if (getIsAttached()) {
            if ((iCalculateNodeKindSetFromIncludingDelegates & NodeKind.m4081constructorimpl(2)) != 0 && (kindSet$ui_release & NodeKind.m4081constructorimpl(2)) == 0) {
    ...
    protected final void undelegate(DelegatableNode instance) {
        ...
        for (Modifier.Node child = this.delegate; child != null; child = child.getChild()) {
            if (child == instance) {
                ...
                if (getIsAttached() && (kindSet$ui_release & NodeKind.m4081constructorimpl(2)) != 0 && (NodeKind.m4081constructorimpl(2) & iCalculateNodeKindSetFromIncludingDelegates) == 0) {
                    NodeChain nodes = DelegatableNodeKt.requireLayoutNode(this).getNodes();
                    getNode().updateCoordinator$ui_release(null);
                    nodes.syncCoordinators();
                    return;
