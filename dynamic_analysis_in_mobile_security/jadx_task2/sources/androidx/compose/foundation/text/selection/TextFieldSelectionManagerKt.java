...
public final class TextFieldSelectionManagerKt {
    ...
    public static final boolean isSelectionHandleInVisibleBound(TextFieldSelectionManager textFieldSelectionManager, boolean z) {
        ...
        return SelectionManagerKt.m927containsInclusiveUv8p0NA(rectVisibleBounds, textFieldSelectionManager.m947getHandlePositiontuRUvjQ$foundation_release(z));
    }
    ...
    public static final long m948calculateSelectionMagnifierCenterAndroidO0kMr_c(TextFieldSelectionManager manager, long j) {
        ...
        if (i == 1 || i == 2) {
            iM4404getStartimpl = TextRange.m4404getStartimpl(manager.getValue$foundation_release().getSelection());
        } else {
            if (i != 3) {
                throw new NoWhenBranchMatchedException();
            }
            iM4404getStartimpl = TextRange.m4399getEndimpl(manager.getValue$foundation_release().getSelection());
        }
        int iOriginalToTransformed = manager.getOffsetMapping().originalToTransformed(iM4404getStartimpl);
        ...
        long jM2420getCenterF1C5BW0 = value.getBoundingBox(iCoerceIn).m2420getCenterF1C5BW0();
        ...
        Offset offsetM945getCurrentDragPosition_m7T9E = manager.m945getCurrentDragPosition_m7T9E();
        if (offsetM945getCurrentDragPosition_m7T9E == null) {
            return Offset.INSTANCE.m2405getUnspecifiedF1C5BW0();
        }
        float fM2390getXimpl = Offset.m2390getXimpl(innerTextFieldCoordinates.mo3874localPositionOfR5De75A(layoutCoordinates, offsetM945getCurrentDragPosition_m7T9E.getPackedValue()));
        ...
        float fCoerceIn = RangesKt.coerceIn(fM2390getXimpl, Math.min(horizontalPosition, horizontalPosition2), Math.max(horizontalPosition, horizontalPosition2));
        if (Math.abs(fM2390getXimpl - fCoerceIn) > IntSize.m5033getWidthimpl(j) / 2) {
            return Offset.INSTANCE.m2405getUnspecifiedF1C5BW0();
        }
        return layoutCoordinates.mo3874localPositionOfR5De75A(innerTextFieldCoordinates, OffsetKt.Offset(fCoerceIn, Offset.m2391getYimpl(jM2420getCenterF1C5BW0)));
    }
