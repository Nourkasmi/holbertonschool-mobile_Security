...
public final class ValidatingOffsetMappingKt {
    ...
    public static final TransformedText filterWithValidation(VisualTransformation visualTransformation, AnnotatedString text) {
        ...
        TransformedText transformedTextFilter = visualTransformation.filter(text);
        return new TransformedText(transformedTextFilter.getText(), new ValidatingOffsetMapping(transformedTextFilter.getOffsetMapping(), text.length(), transformedTextFilter.getText().length()));
    }
}
