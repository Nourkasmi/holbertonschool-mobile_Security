...
public final class LazyStaggeredGridAnimateScrollScope implements LazyAnimateScrollScope {
    ...
    public Integer getTargetItemOffset(int index) {
        ...
        long jMo654getOffsetnOccac = lazyStaggeredGridItemInfoFindVisibleItem.getOffset();
        return Integer.valueOf(this.state.getIsVertical() ? IntOffset.m4992getYimpl(jMo654getOffsetnOccac) : IntOffset.m4991getXimpl(jMo654getOffsetnOccac));
    }
    ...
    public float expectedDistanceTo(int index, int targetScrollOffset) {
        ...
        int iM5032getHeightimpl = 0;
        for (int i = 0; i < size; i++) {
            LazyStaggeredGridItemInfo lazyStaggeredGridItemInfo = visibleItemsInfo.get(i);
            iM5032getHeightimpl += this.state.getIsVertical() ? IntSize.m5032getHeightimpl(lazyStaggeredGridItemInfo.getSize()) : IntSize.m5033getWidthimpl(lazyStaggeredGridItemInfo.getSize());
        }
        int size2 = (iM5032getHeightimpl / visibleItemsInfo.size()) + layoutInfo.getMainAxisItemSpacing();
        ...
        return ((size2 * laneCount$foundation_release) + iMin) - getFirstVisibleItemScrollOffset();
    }
