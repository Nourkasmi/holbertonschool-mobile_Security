...
public final class LazyGridAnimateScrollScope implements LazyAnimateScrollScope {
    ...
    public Integer getTargetItemOffset(int index) {
        ...
        if (this.state.getIsVertical()) {
            iM4991getXimpl = IntOffset.m4992getYimpl(lazyGridItemInfo2.getOffset());
        } else {
            iM4991getXimpl = IntOffset.m4991getXimpl(lazyGridItemInfo2.getOffset());
        }
        return Integer.valueOf(iM4991getXimpl);
    }
        ...
        while (i < visibleItemsInfo.size()) {
            ...
            if (iIntValue == -1) {
                ...
                while (i < visibleItemsInfo.size() && function1.invoke(Integer.valueOf(i)).intValue() == iIntValue) {
                    if (isVertical) {
                        iM5033getWidthimpl = IntSize.m5032getHeightimpl(visibleItemsInfo.get(i).getSize());
                    } else {
                        iM5033getWidthimpl = IntSize.m5033getWidthimpl(visibleItemsInfo.get(i).getSize());
                    }
                    iMax = Math.max(iMax, iM5033getWidthimpl);
                    i++;
                }
                i2 += iMax;
                i3++;
